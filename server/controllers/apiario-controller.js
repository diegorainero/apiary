...
exports.apiarioCreate = async (req, res) => {
  ...
  knex('apiario')
    ...
    .then(() => {
      ...
      res.json({ message: `Apiario \'${req.body.name}\' sito in ${req.body.location}.` })
    })
    .catch(err => {
      // Send a error message in response
...
exports.apiarioDelete = async (req, res) => {
  ...
  knex('apiario')
    ...
    .catch(err => {
      // Send a error message in response
      res.json({ message: `There was an error deleting ${req.body.id} apiario: ${err}` })
    })
}
...
exports.apiarioReset = async (req, res) => {
  ...
  knex
    ...
    .catch(err => {
      // Send a error message in response
      res.json({ message: `Ci sono stati errori nella cancellazione della lista degli apiari: ${err}.` })
    })
}