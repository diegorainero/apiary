...
exports.arnieAll = async (req, res) => {
	...
	console.log('DENTRO ARNIE %o', req.params);
...
exports.arnieCreate = async (req, res) => {
	...
	knex('arnia')
		...
		.then(() => {
			...
			res.json({
				message: `Arnia \'${req.body.name}\' sito in ${req.body.location_id}.`,
			});
		})
		.catch((err) => {
			// Send a error message in response
...
exports.arnieDelete = async (req, res) => {
	...
	knex('arnia')
		...
		.catch((err) => {
			// Send a error message in response
			res.json({
				message: `There was an error deleting ${req.body.id} arnia: ${err}`,
			});
		});
};
...
exports.arnieReset = async (req, res) => {
	...
	knex.select('*') // select all records
		...
		.catch((err) => {
			// Send a error message in response
			res.json({
				message: `Ci sono stati errori nella cancellazione della lista delle arnie: ${err}.`,
			});
		});
};
