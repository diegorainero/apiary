...
export const Arnie = () => {
  ...
  const fetchArnia = async () => {
    // Send GET request to 'apiari/arnie/all' endpoint
    axios
      .get('http://localhost:4001/apiari/arnie/all')
      .then(response => {
      ...
      .catch(error => console.error(`There was an error retrieving the arnie list: ${error}`))
  }
  ...
  const handleArniaCreate = () => {
    ...
    axios
      ...
      .catch(error => console.error(`There was an error creating the ${name} arnia: ${error}`))
  }
  ...
  const handleArnieRemove = (id: number, title: string) => {
    ...
    axios
      ...
      .catch(error => console.error(`There was an error removing the ${name} apiario: ${error}`))
  }
      ...
      <div className="apiario-list-form">
        <div className="form-wrapper" onSubmit={handleArniaSubmit}>
          <div className="form-row">
            ...
            <fieldset>
              <label className="form-label" htmlFor="location_id">Scegliere Luogo:</label>
              <input className="form-input" type="text" id="location_id" name="location_id" value={location_id} onChange={(e) => setLocationId(e.currentTarget.value)} />
            </fieldset>
          </div>
        </div>

        <button onClick={handleArniaSubmit} className="btn btn-add">Aggiungi un'arnia</button>
      </div>

      {/* Render bookshelf list component */}
      <ArniaList arnie={arnia} loading={loading} handleArniaRemove={handleArnieRemove} />

      {/* Show reset button if list contains at least one book */}
      {arnia.length > 0 && (
        <button className="btn btn-reset" onClick={handleListReset}>Reset Arnia list.</button>
      )}
    </div>
  )
}