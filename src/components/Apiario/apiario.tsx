...
export const Apiario = () => {
  ...
  const fetchApiario = async () => {
    ...
    axios
      ...
      .catch(error => console.error(`There was an error retrieving the apiario list: ${error}`))
  }
  ...
  const handleApiarioCreate = () => {
    ...
    axios
      ...
      .catch(error => console.error(`There was an error creating the ${name} apiario: ${error}`))
  }

  // Submit new book
  const handleApiarioSubmit = () => {
    // Check if all fields are filled
    if (name.length > 0 && location.length > 0 && lat.length > 0 && long.length > 0) {
      // Create new book
      handleApiarioCreate()
  ...
  const handleApiarioRemove = (id: number, title: string) => {
    ...
    axios
      ...
      .catch(error => console.error(`There was an error removing the ${title} apiario: ${error}`))
  }
  ...
  const HandleApiarioSelect = (id: string, title: string) => {
    ...
    history.push("/apiari/arnie/all?arnia="+id);
    // axios
  ...
  const handleListReset = () => {
    ...
    axios.put('http://localhost:4001/apiari/reset')
    .then(() => {
      // Fetch all books to refresh
      // the books on the bookshelf list
      fetchApiario()
    })
    .catch(error => console.error(`There was an error resetting the apiario list: ${error}`))
  }
      ...
      <div className="apiario-list-form">
        <div className="form-wrapper" onSubmit={handleApiarioSubmit}>
          <div className="form-row">
            <fieldset>
              <label className="form-label" htmlFor="name">Inserire nome:</label>
              <input className="form-input" type="text" id="name" name="name" value={name} onChange={(e) => setName(e.currentTarget.value)} />
            </fieldset>

            <fieldset>
              <label className="form-label" htmlFor="location">Inserire Luogo:</label>
          ...
          <div className="form-row">
            <fieldset>
              <label className="form-label" htmlFor="lat">Inserire Latitudine:</label>
              <input className="form-input" type="text" id="lat" name="lat" value={lat} onChange={(e) => setLat(e.currentTarget.value)} />
            </fieldset>

            <fieldset>
              <label className="form-label" htmlFor="long">Inserire Longitudine:</label>
              <input className="form-input" type="text" id="long" name="long" value={long} onChange={(e) => setLong(e.currentTarget.value)} />
            </fieldset>
          </div>
        </div>

        <button onClick={handleApiarioSubmit} className="btn btn-add">Aggiungi un apiario</button>
      </div>

      {/* Render bookshelf list component */}
      <ApiarioList apiari={apiari} loading={loading} handleApiarioRemove={handleApiarioRemove} HandleApiarioSelect={HandleApiarioSelect} />

      {/* Show reset button if list contains at least one book */}
      {apiari.length > 0 && (
        <button className="btn btn-reset" onClick={handleListReset}>Reset Apiario list.</button>
      )}
    </div>
  )
}